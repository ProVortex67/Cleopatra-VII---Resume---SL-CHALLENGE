...
function App() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => setProgress(Math.min(100, Math.max(0, (window.scrollY / Math.max(1, document.documentElement.scrollHeight - window.innerHeight)) * 100)));
    window.addEventListener('scroll', onScroll, { passive: true });
    return () => { observer.disconnect(); window.removeEventListener('scroll', onScroll); };
  }, []);

  const scores = assessmentActive ? assessment : assessment.map(([label, value]) => [label, Math.round(Number(value) * .88)]);
  ...
  return <div className="site">
    ...
    <main id="top">
      <section id="profile" className="hero">
        <div className="hero-copy reveal"><p className="eyebrow"><span /> EXECUTIVE DOSSIER · ALEXANDRIA · 69—30 BCE</p><p className="kicker">SOVEREIGN / STRATEGIST / DIPLOMAT</p><h1>Cleopatra <em>VII</em></h1><p className="dek">Leadership under pressure,<br />translated for the modern boardroom.</p><div className="hero-actions"><button className="gold-btn" onClick={() => go('assessment')}>Read the executive assessment <ArrowUpRight size={16} /></button><button className="text-btn" onClick={() => go('achievements')}>Explore the record <ChevronDown size={15} /></button></div><div className="metrics"><div><strong>21</strong><span>years of rule</span></div><div><strong>8</strong><span>languages attributed</span></div><div><strong>∞</strong><span>cultural influence</span></div></div></div>
        <div className="portrait-wrap reveal" aria-label="Stylized editorial portrait of Cleopatra VII"><div className="orbit orbit-a" /><div className="orbit orbit-b" /><div className="halo" /><div className="portrait"><div className="crown">𓂀</div><div className="face"><div className="eye e1" /><div className="eye e2" /><div className="nose" /><div className="mouth" /></div><div className="hair" /></div><div className="seal"><Crown size={15} /><span>ROYAL<br />ARCHIVE</span></div><div className="portrait-note"><span>01</span><b>THE LAST<br />PTOLEMAIC</b></div></div>
      </section>
      ...
      <section id="achievements" className="section reveal"><div className="section-top"><p className="section-index">01 / 05</p><p className="eyebrow">TRACK RECORD</p></div><div className="section-head"><h2>Results that <em>changed the map.</em></h2><p>A career defined by compressed timelines, high-stakes stakeholders and the ability to convert instability into leverage.</p></div><div className="timeline">{achievements.map(([year, title, desc], i) => <article className="achievement" key={year}><div className="year">{year}</div><div className="timeline-index">0{i + 1}</div><div className="achievement-copy"><h3>{title}</h3><p>{desc}</p></div><ArrowUpRight className="row-arrow" size={17} /></article>)}</div></section>
      <section id="leadership" className="dark-panel reveal"><div className="panel-intro"><div className="section-top"><p className="section-index">02 / 05</p><p className="eyebrow">LEADERSHIP STYLE</p></div><h2>High-context.<br /><em>High conviction.</em></h2><p>Her edge was not one trait. It was the combination: cultural intelligence, personal presence, strategic patience and the willingness to act decisively when the window opened.</p></div><div className="principles"><article><span>01</span><Shield /><h3>Resilient authority</h3><p>Survived exile, civil conflict and shifting Roman power without surrendering strategic agency.</p></article><article><span>02</span><Globe2 /><h3>Cultural fluency</h3><p>Used language, symbolism and local knowledge as instruments of diplomacy and statecraft.</p></article><article><span>03</span><Gem /><h3>Alliance architecture</h3><p>Built relationships around shared interests, incentives and timing rather than sentiment alone.</p></article></div></section>
    ...
    </main><footer><span>𓂀 CLEOPATRA VII · EXECUTIVE ARCHIVE</span><span>ALEXANDRIA · 2026 EDITION</span></footer>
  </div>;
}
